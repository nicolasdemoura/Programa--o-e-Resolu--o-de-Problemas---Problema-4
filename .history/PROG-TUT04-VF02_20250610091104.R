...
gg_ibov <- ggplot(data.frame(lag = acf_ibov$lag, acf = acf_ibov$acf), aes(x = lag, y = acf)) +
  geom_bar(stat = "identity", fill = "#d62728") +
  labs(title = "ACF - Retornos Simples Ibovespa", x = "Lag", y = "ACF") +
  theme_minimal()
gg_template_save(gg_ibov, "acf_ibov_ret_simple.png")

# S&P 500
acf_sp <- acf(df$sp_ret_simple, plot = FALSE)
gg_sp <- ggplot(data.frame(lag = acf_sp$lag, acf = acf_sp$acf), aes(x = lag, y = acf)) +
  geom_bar(stat = "identity", fill = "#1f77b4") +
  labs(title = "ACF - Retornos Simples S&P 500", x = "Lag", y = "ACF") +
  theme_minimal()
gg_template_save(gg_sp, "acf_sp500_ret_simple.png")
